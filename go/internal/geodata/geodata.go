...
// Package geodata owns the platform-neutral Geo seed to sing-box rule-set
// generation.  Platform adapters only provide command execution and paths.
package geodata
...
func Catalog(ctx context.Context, runner Runner, kind, seedDirectory, geoViewBinary string) ([]string, error) {
...
func EnsureRules(ctx context.Context, runner Runner, ruleSets []compiler.GeoRuleSet, options Options) (returnErr error) {
	...
	if generationReady(current, release, manifest, ruleSets) {
...
func generationReady(current string, release, manifest []byte, ruleSets []compiler.GeoRuleSet) bool {

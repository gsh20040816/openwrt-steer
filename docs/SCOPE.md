...
- schema 7 Canonical Intent：主配置、Bootstrap、节点、订阅、逻辑路由、DNS Profile、本地代理和规则；
...
- UCI schema 7 codec 和 UCI 订阅节点持久化；
...
- alpha 期间允许修正接口命名或职责泄漏，但不得借机增加功能。
- OpenWrt 正常使用至少连续一周，Apply、开机启动、DNS、TCP/UDP、IPv4/IPv6、订阅和显式测试没有确认回归后，才考虑 `v0.4.0`。
- 稳定版之后再从 Linux 适配器开始跨平台实现；macOS 排在 Linux 之后。

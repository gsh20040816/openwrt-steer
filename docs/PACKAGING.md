...
0.4.3 面向 OpenWrt 25.12.5 x86/64。GitHub Actions 使用固定官方 SDK 镜像构建；tag 发布只复用同一 commit 已成功生成的 master 构建，不重新编译。

## 包与所有权

| 包 | 版本 | 所有内容 |
|---|---|---|
| `steer-openwrt` | `0.4.3-r1` | `/usr/sbin/steer`、默认 UCI、procd init、Apply/OpenWrt 适配器 |
| `luci-app-steer` | `0.4.3-r1` | LuCI 页面、ucode RPC、ACL |
| `luci-i18n-steer-zh-cn` | `0.4.3-r1` | 简体中文翻译 |

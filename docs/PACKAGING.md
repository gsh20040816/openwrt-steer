...
版本所有权彼此独立：Steer、LuCI 和 OpenWrt `steer` APK 跟随 `v0.6.3`；`steer-geodata` 使用独立数据版本；geoview 和 sing-box 跟随各自上游或发行版。

## OpenWrt

0.6.3 面向 OpenWrt 25.12.5 x86/64：

| 包 | 版本 | 所有内容 |
|---|---|---|
| `steer` | `0.6.3-r1` | `/usr/sbin/steer`、默认 UCI、procd init、Apply/OpenWrt 适配器 |
| `luci-app-steer` | `0.6.3-r1` | LuCI 页面、ucode RPC、ACL |
| `luci-i18n-steer-zh-cn` | `0.6.3-r1` | 简体中文翻译 |

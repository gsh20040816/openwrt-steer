...
当前稳定版本是 `v0.8.2`。
当前预览版本是 `v0.9.0-alpha.2`；OpenWrt APK 将同一版本规范化为 `0.9.0_alpha2`，Arch `pkgver` 使用 `0.9.0alpha2`，Git tag、Linux 与 macOS 构件保留公开 SemVer 拼写。
...
`v0.9.0-alpha.2` 面向 OpenWrt 25.12.5 x86/64：

| 包 | 版本 | 所有内容 |
|---|---|---|
| `steer` | `0.9.0_alpha2-r1` | 控制器、默认 UCI、procd init、完整只读 SRS seed |
| `luci-app-steer` | `0.9.0_alpha2-r1` | LuCI 页面、ucode RPC、ACL |
| `luci-i18n-steer-zh-cn` | `0.9.0_alpha2-r1` | 简体中文翻译 |

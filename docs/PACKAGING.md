...
0.5.0-alpha.1 面向 OpenWrt 25.12.5 x86/64。GitHub Actions 使用固定官方 SDK 镜像构建；tag 发布只复用同一 commit 已成功生成的 master 构建，不重新编译。

## 包与所有权

| 包 | 版本 | 所有内容 |
|---|---|---|
| `steer` | `0.5.0_alpha1-r1` | `/usr/sbin/steer`、默认 UCI、procd init、Apply/OpenWrt 适配器 |
| `luci-app-steer` | `0.5.0_alpha1-r1` | LuCI 页面、ucode RPC、ACL |
| `luci-i18n-steer-zh-cn` | `0.5.0_alpha1-r1` | 简体中文翻译 |
...
Go 源码位于仓库根的 `go/`，包定义从同一 feed 的兄弟目录复制构建输入。`steer` 包通过 `PROVIDES`、`CONFLICTS` 和 `REPLACES` 元数据原子替换旧 `steer-openwrt` 包；代码树中不保留旧目录或兼容副本。
...
APK 版本为 `0.5.0_alpha1-r1`，Git tag 为 `v0.5.0-alpha.1`。发布流程为：

1. 在 master 提交并推送完整原子变更；
2. 等待该 commit 的 `Build OpenWrt packages` 成功；
3. 给同一 commit 打 `v0.5.0-alpha.1` tag 并推送；
4. `Publish tagged release` 下载 master 构建、校验 SHA256/commit，再创建预发布 Release；
5. 从 Release 下载 APK 安装到目标路由器，执行正常配置校验和健康检查。

不得从本地未留证据的二进制发版，也不得给 tag 重新构建一套可能不同的产物。`v0.5.0-alpha.1` 复用同一提交已经成功生成并留存证据的 master 构建。

...
当前稳定版本为 **0.4.3**，当前已发布预览版为 **0.5.0-alpha.1**，主线正在准备 **0.5.0-alpha.2**，公开配置为 **schema 7**。OpenWrt 25.12.5 x86/64 是稳定发行平台；Linux systemd 适配器已实现源码基线，覆盖主机及 VM/Docker 公网转发流量，暂不打包；macOS 尚未提供适配器。

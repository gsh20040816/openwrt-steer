...
func TestRenderMinimalDNSAndMACShims(t *testing.T) {
	plan := compiler.Plan{Resources: compiler.Resources{DNSPort: 1053, AutoRedirectOutputMark: 0x2024, MACMark: 0x2026, MACTable: 2023, MACPriority: 8999,
...
func TestRenderFirewallDoesNotRequireResolvedDevices(t *testing.T) {
	if _, err := RenderFirewall(compiler.Plan{}); err != nil {
		t.Fatal(err)
	}
}

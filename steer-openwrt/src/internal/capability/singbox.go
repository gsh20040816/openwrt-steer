...
func Inspect(binary string, required []string) Report {
	output, err := exec.Command(binary, "version").CombinedOutput()
	if err != nil {
		return Report{Errors: []string{fmt.Sprintf("execute %s version: %v: %s", binary, err, strings.TrimSpace(string(output)))}}
	}
	return Parse(string(output), required)
}
